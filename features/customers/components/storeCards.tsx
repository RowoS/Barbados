    ...
    return (
        ...
        >
            <div className="relative h-48 overflow-hidden bg-gray-100">
                {store.store_logo ? (
                    <img src={store.store_logo} alt={store.store_name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                ) : (
                ...
                <button
                    onClick={(e) => { e.stopPropagation(); e.preventDefault(); onToggleFavorite(store.id); }}
                ...
                >
                    <Heart className={`w-4 h-4 ${isFavorite ? "fill-[#FF6B35] text-[#FF6B35]" : "text-gray-400"}`} />
                </button>
                {store.barangay_name && (
